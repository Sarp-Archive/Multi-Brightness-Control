...
import sys
if "-s" not in sys.argv[1:]: print("Multi Brightness Control")
...
if "--check-packages" in sys.argv[1:]:
    ...
    import subprocess
    if "-s" not in sys.argv[1:]: print("Checking if need to install packages...")
    ...
    if to_install:
        if "-s" not in sys.argv[1:]: print("%d packages is not installed." % len(to_install))
        for pkg in to_install:
            if "-s" not in sys.argv[1:]: print("Installing " + pkg)
            subprocess.check_call([sys.executable, "-m", "pip", "install", pkg]) # Install packages via pip

# Import necessary packages
if "-s" not in sys.argv[1:]: print("Importing packages...")
...
if "--skip-count-validation" not in sys.argv[1:]: # Check for available monitor count
    ...
    while monitor_count < 0: # Wait for other monitors to be connected
        if "-s" not in sys.argv[1:]: print("Attempt %d: No external monitors found." % wait)
        time.sleep(1)
        if wait < 30: # Wait until 30 failed attempts
            monitor_count = len(sbc.list_monitors())
            wait += 1
        else:
            if "-s" not in sys.argv[1:]: print("Attempt %d: %d seconds passed and still no monitors connected. Terminating..." % wait)
            terminate(None) # No reason to waste system resources if there is no external monitor connected

# Print display count
if "-s" not in sys.argv[1:]: print("%d monitors found." % monitor_count)

# Set current brightness as last value
last = sbc.get_brightness(display=0)

# Print current brightness
if "-s" not in sys.argv[1:]: print("Primary display brightness is " + str(last) + "%.")

# Extract icon to file
if "--no-extract-icon" not in sys.argv[1:]:
    if "-s" not in sys.argv[1:]: print("Extracting icon to file...")
...
if "--no-systray-icon" not in sys.argv[1:]:
    ...
    if "-s" not in sys.argv[1:]: print("Starting system tray icon...")
...
if "-s" not in sys.argv[1:]: print("Starting main loop...")
while True:
